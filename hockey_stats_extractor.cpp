...
static std::string gemini(const std::string& key, const std::string& img)
{
    const std::string PROMPT=
        ...
        "Player|TOI|Goals|Assists|Points|PlusMinus|Shots|ShotPct|PPTime|PenTime|Hits|PPG|SHG\n"
        "Every subsequent row = one skater, exactly 13 pipe-separated fields.\n"
        "Missing column -> output 0 for every player in that column.\n"
        "PenTime in MM:SS. PPG = powerplay goals. SHG = shorthanded goals. No spaces around pipes.\n";
                    ...
                    int gn, bool append_col=false)
{
    ...
    int cip =ci("player"),  cig=ci("goals"),   cia=ci("assists"),
        cip2=ci("points"),  cipm=ci("plusminus"), cis=ci("shots"),
        cisp=ci("shotpct"), cippt=ci("pptime"), cipent=ci("pentime"),
        cih =ci("hits"),    cippg=ci("ppg"),    cishg=ci("shg");

    // Append season_scores.txt
    // Format: game|player|goals|assists|points|plusminus|shots|shotpct|pptime|pentime|hits|ppg|shg
    ...
    for(size_t r=1;r<rows.size();r++){
        ...
        sf<<gn<<"|"<<g(cip)<<"|"<<g(cig)<<"|"<<g(cia)<<"|"<<g(cip2)<<"|"
          ...
          <<g(cipent)<<"|"<<g(cih)<<"|"<<g(cippg)<<"|"<<g(cishg)<<"\n";
    }
